...
BW2 = imbinarize(BW1,0.9);       %对图像自动二值化即可。 BW2二值图

BW2=imcomplement(BW2);    %颜色翻转
BW2 = bwareaopen(BW2,2500,8);  %去文字
...
for i=2:len
    ...
    if ~isempty(houxuan)
        if rectangles(1,i)==1
            a1=ceil(status(i).BoundingBox(1))-3;
            a2=ceil(status(i).BoundingBox(1)+status(i).BoundingBox(3))+2;
            b1=ceil(status(i).BoundingBox(2))-3;
            b2=ceil(status(i).BoundingBox(2)+status(i).BoundingBox(4))+2;
            COPY3(b1:b2,a1:a2)=1;
        end
    end
end
...
for i=2:len
    ...
    if ~isempty(houxuan)
        if circles(1,i)==1
            a1=ceil(status(i).BoundingBox(1))-5;
            a2=ceil(status(i).BoundingBox(1)+status(i).BoundingBox(3))+5;
            b1=ceil(status(i).BoundingBox(2))-5;
            b2=ceil(status(i).BoundingBox(2)+status(i).BoundingBox(4))+5;
...
for ts=1:mm
    ...
    for i1=b1:b2
        for j1=a1:a2
            B=A(i1-1:i1+1,j1-1:j1+1);
            if B(2,2)==1                               %8邻域像素点为3
                if (B(2,1)==1&&B(1,2)==1&&B(2,3)==1)||(B(2,1)==1&&B(3,2)==1&&B(2,3)==1)||(B(1,2)==1&&B(2,1)==1&&B(3,2)==1)||(B(1,2)==1&&B(2,3)==1&&B(3,2)==1)||(B(1,2)==1&&B(2,3)==1&&B(3,1)==1)||(B(1,1)==1&&B(2,3)==1&&B(3,2)==1)||(B(1,3)==1&&B(2,1)==1&&B(3,2)==1)
                    KK=[j1,i1];
                    KKs{ii}=KK;
                    ii=ii+1;
                    jointpoints{ts}=KKs;
...
for ts=1:num2
    ...
    A=imerode(A,se3);
    A = bwareaopen(A,20,8);
    [arrowL,arrownum] = bwlabel(A,8);
    arrowstatus=regionprops(arrowL,'BoundingBox');
    arrowcentroid = regionprops(arrowL,'Centroid');
    for i=1:arrownum
        x0=arrowcentroid(i,1).Centroid(1,1);
        y0=arrowcentroid(i,1).Centroid(1,2);
        K=[x0,y0];
        arrowhouxuan{ii}=K;
        ii=ii+1;
        arrowhousuans{ts}=arrowhouxuan;
    end
    arrowhouxuan=[];
end
%%%%%%%%%%%%%%%%%%%去掉和连接点相重的部分%%%%%%%%%%%%%%%%%%%%
...
for i = 1:length(jointpoints)
    points=jointpoints{i};
    if ~isempty(points)
        for j=1:length(points)
            pp=points{j};
            jointpointCOPY{ii}=pp;
            ii=ii+1;
        end
    end
end
ii=1;
for i=1:num2
    arrowhouxuan=arrowhousuans{i};
    if ~isempty(arrowhouxuan)
        for j=1:length(arrowhouxuan)
            arrowpoint=arrowhouxuan{j};
            x1=arrowpoint(1,1);
            y1=arrowpoint(1,2);
            for i1=1:length(jointpointCOPY)
                jointpoint=jointpointCOPY{i1};
                x0=jointpoint(1,1);
                y0=jointpoint(1,2);
...
% num2=length(arrowlines);
% for i=1:num2
%      imwrite(arrowlines{i},['test',num2str(i),'.png']);
% end
%%
...
%%%%%%%%%%%%%%%画出端点位置%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%画出箭头位置%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for i=1:length(arrowhousuans)
    zuobiaos=arrowhousuans{i};
    for j=1:length(zuobiaos)
        if ~isempty(zuobiaos{j})
            jj=zuobiaos{j};
            x0=jj(1,1);
            y0=jj(1,2);
            plot(x0,y0,'yo','LineWidth',5,'MarkerFaceColor','b');
        end
    end
end
